...
const EditProduct = () => {
  ...
  useEffect(() => {
    if (!editProduct) {
      ...
      setStockLocationId(editProduct.stock_location_id);

      setPrice(editProduct.price);
      setBuyingPrice(editProduct.buying_price);

      setDiscountAmount(editProduct.discountAmount);
      setDiscountType(editProduct.discount_type );
      setDiscountDate(editProduct.discount_date);
      setDiscountAmount(editProduct.discountAmount);
      setStock(editProduct.stock);
      setCode(editProduct.code);


      setStatus(editProduct.is_published);

      setShort_desc(editProduct.short_desc );
    ...
    const fetchVariation = async () => {
      try {
        const response = await axios.get(
          `https://admin.ezicalc.com/api/product/variation/values/${clientId}`,
          {
            headers: {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://admin.ezicalc.com/api/product/update",
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <div className="md:col-span-1 order-1 md:order-2">
            <div className="mb-4 bg-white p-4 shadow-[0_3px_10px_rgb(0,0,0,0.2)]">
              ...
              <ProductSelectedBusiness
               ...
               businessId ={editProduct?.business_id} 
                ...
                >
                  ...
                  >
                    {image || editProduct?.image ? (
                      <image
                        href={`https://admin.ezicalc.com/public/storage/product/${image}`}
                        width="90"
                        height="70"
                      />
                    ) : (
